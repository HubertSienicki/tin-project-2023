...
public class UserRepository : IUserRepository
{
    ...
    public async Task<User?> GetByUsername(string username)
    {
        ...
        try
        {
            ...
            await using var command = connection.CreateCommand();
            command.CommandText =
                $"SELECT u1.Id, u1.username, u1.Email, u1.password, r1.roleId as RoleId, r1.Name as RoleName FROM Users u1 JOIN Roles r1 ON r1.roleId = u1.role_id WHERE u1.username = \'{username}\'";
            await using var reader = command.ExecuteReader();
            if (reader.Read())
            {
                var user = new User
